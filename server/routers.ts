...
import { answerGroundedly, extractPdf, gradeQuizAnswers, makeChunks, makeFlashcards, makeQuiz, makeSummary } from "./study";
...
export const appRouter = router({
  ...
  study: router({
    ...
    uploadPdf: protectedProcedure.input(z.object({ subjectId: z.number().int().positive(), fileName: z.string().min(1).max(255), dataUrl: z.string().min(40) })).mutation(async ({ ctx, input }) => {
      ...
      const chunks = parsed.pages.flatMap((page, pageIndex) => makeChunks(page.text, page.pageNumber, pageIndex * 100));
      ...
      const material = await db.createMaterialWithChunks(ctx.user.id, {
        ...
        extractedText: parsed.pages.map(page => page.text).join("\n\n"),
        processingStatus: chunks.length ? "ready" : "needs_attention",
        chunks,
      });
      return { material: requireOwned(material, "Your PDF could not be saved."), truncated: parsed.truncated };
    }),
